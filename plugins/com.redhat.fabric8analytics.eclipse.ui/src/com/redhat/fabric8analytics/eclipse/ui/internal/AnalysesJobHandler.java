...
import com.redhat.fabric8analytics.eclipse.ui.internal.view.StackAnalysisHandler;
...
public class AnalysesJobHandler {
	...
	public void analyze() {
		URL url;
		try {
			url = new URL("platform:/plugin/"+Fabric8AnalysisUIActivator.PLUGIN_ID+"/templates/index.html");
		...
		if (editorComposite == null) {
			IViewPart mainView = StackAnalysisHandler.getView();
	...
	private void syncWithUi(String analysisURL) {
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				if (editorComposite == null) {
					((StackAnalysesView) StackAnalysisHandler.getView()).updatebrowserUrl(analysisURL);
				} else {
	...
	private void disposeBrowsers() {
		Display.getDefault().syncExec(new Runnable () {
			public void run() {
				if(editorComposite == null) {
					((StackAnalysesView) StackAnalysisHandler.getView()).disposebrowserUrl();
				} else {
