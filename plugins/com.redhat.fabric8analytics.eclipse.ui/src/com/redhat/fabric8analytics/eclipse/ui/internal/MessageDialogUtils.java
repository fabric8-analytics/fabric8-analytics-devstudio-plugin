...
public class MessageDialogUtils {
	...
	public static AnalyticsAuthData proptForLogin(IProgressMonitor monitor) {
		...
		if(dialog.open() == Window.OK) {
			try {
				...
				Fabric8AnalysisUIActivator.getDefault().logError("Error while storing Fabric8Analytics data", e);
			}
		}
		return null;
	}
	...
	public static void displayErrorMessage(String message) {
		Fabric8AnalysisUIActivator.getDefault().logError(message);
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
	...
	public static void displayErrorMessage(String message, Throwable t) {
		Fabric8AnalysisUIActivator.getDefault().logError(message, t);
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				MessageDialog.openError(PlatformUI.getWorkbench().getDisplay().getActiveShell(), "Fabric8 error message", message);
			}
